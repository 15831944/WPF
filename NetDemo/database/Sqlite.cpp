...
*/
bool CSqlite::Open(std::string lstrFile,bool lbCreate ,bool lbWrite )
{
	try
	{
		int  lnFlags      =  0 ;
...
int CSqlite::Count(std::string lstrTable , std::string lstrCondition)
{
	int lnCount = 0 ;
	if(m_pDb)
	{
...
* @see              关联          
*/
bool CSqlite::Pragmas(std::string lstrCommands)
{
	try 
	{
		if(m_pDb)
...
* @return           返回值描述  
*/
bool CSqlite::IsExist(std::string lstrTableName)
{
	sqlite3_stmt*       lstmt ;
	const char*         ltail ;
	std::string     lstrSQL ;
...
* @return           返回值描述  
*/
bool CSqlite::IsTempExist(std::string lstrTableName)
{
	sqlite3_stmt*       lstmt ;
	const char*         ltail ;
	std::string     lstrSQL ;
...
* @return           返回值描述  
*/
bool CSqlite::IsExist(std::string lstrDBName, std::string lstrTableName)
{
	sqlite3_stmt*       lstmt ;
	const char*         ltail ;
	std::string     lstrSQL ;
...
* @see              关联          
*/
void CSqlite::SetAttribute()
{ 
	std::string         lstrSQL ;
	SYSTEM_INFO         lsiSysInfo;
	char                ltcBuf[MAX_PATH] ;
...
bool CSqlite::ExeSql(std::string lstrSql )
{
	int result = 0;
	char *pErrInfo = NULL;
